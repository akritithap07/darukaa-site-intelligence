...
export default function AppNavbar({ project, site }) {
  ...
  return (
    <header className="bg-slate-800/95 backdrop-blur-md border-b border-slate-700/80 px-6 py-3.5 flex items-center justify-between sticky top-0 z-50 shadow-md shrink-0">
        ...
        <nav className="flex items-center space-x-2 text-xs text-slate-300">
          ...
          {project && (
            <>
              ...
              <Link to={`/projects/${project.id}`} className="hover:text-white font-medium text-slate-200 transition">
                {project.name}
              </Link>
            </>
          )}
      ...
      <div className="flex items-center space-x-4">
        <button
          ...
          className="flex items-center space-x-1.5 text-xs font-medium text-slate-400 hover:text-rose-400 bg-slate-900/50 hover:bg-slate-900 border border-slate-700/60 px-3 py-1.5 rounded-md transition cursor-pointer"
        >
